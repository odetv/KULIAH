...
struct data{
	char nim[11], nama[51], jk[10];
	double tahunlahir, tahunmasuk;
	float ipk;
};data mhs[100];
...
void lihatdata(){
	...
	else
	{
		...
		file.open("Database Mahasiswa.txt", ios::trunc);
		j=1;
		for(i=0; i<savedata; i++)
		{
			...
			file << "Data Ke-" << j << "\n";
			file << "NIM\t\t: " << mhs[i].nim << "\n";
			file << "Nama\t\t: " << mhs[i].nama << "\n";
			file << "Jenis Kelamin\t: " << mhs[i].jk <<  "\n";
			file << "Tahun Lahir\t: " << mhs[i].tahunlahir <<  "\n";
			file << "Tahun Masuk\t: " << mhs[i].tahunmasuk << "\n";
			file << "IPK\t\t: " << mhs[i].ipk << endl;
			j++;
	...
	data=fopen("Database Mahasiswa.txt", "r");
